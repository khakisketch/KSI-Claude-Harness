...
- **웹 도구가 없다.** 외부 문서 조회는 `Explore`(haiku)의 일이다 — opus·xhigh로 문서를 읽는 건 가장 비싼 조합이고, read-only 검증자에 외부 전송 경로를 열어둘 이유도 없다. 라이브러리 실제 동작 확인이 필요하면 "외부 확인 필요: <무엇을>"으로 위임자에게 올린다.
