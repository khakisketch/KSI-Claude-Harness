...
스킬은 **모델이 그냥은 하지 않는 절차**만 담습니다. "넓게 생각해라" 같은 건 이미 하는 일이라 스킬로 만들지 않습니다.
...
> 가장 약한 모델에 가장 넓은 쓰기 권한을 주지 않습니다 — **읽기는 `Explore`, 쓰기는 `worker` 이상**이 맡습니다.
...
요청이 들어오면 메인이 판단하고, 무거운 일은 등급에 맞는 일꾼에게 넘기고, 결과는 감리를 통과해야 완료가 됩니다.

```mermaid
%%{init: {'flowchart': {'curve': 'linear', 'nodeSpacing': 45, 'rankSpacing': 60}}}%%
flowchart LR
    REQ([요청]) --> MAIN[메인<br/>판단 · 분해 · 통합]
    MAIN -->|조사| EX[Explore · Haiku]
    EX -->|결론만| MAIN
    MAIN -->|구현| WK[worker · Sonnet]
    WK --> RV[reviewer · Opus<br/>검측]
    RV -->|반려| WK
    RV -->|통과| DONE([완료])
...
**메인의 문맥은 깨끗하게 유지됩니다** — 조사 에이전트가 파일을 훑어도 그 원문은 메인에 들어오지 않고, 결론만 돌아옵니다.
...
- 세 에이전트에 역할별 모델과 사고량을 **고정**했습니다. 호출할 때 지정을 잊어도 상속이 일어나지 않습니다.
- 검증의 사고량은 오히려 낮추고, 구현에 몰았습니다 — **결함은 나중에 찾는 것보다 처음부터 안 만드는 쪽이 쌉니다.**
- 비용을 줄이는 방법은 싼 모델로 낮추는 게 아니라 **자리에 맞는 모델을 쓰는 것**입니다. 잘못된 등급이 만든 재작업이 가장 비쌉니다.

### 2. 메인의 문맥을 더럽히지 않는다

대화가 길어질수록 느려지고, 비싸지고, 판단이 흐려집니다.
문맥을 가장 빨리 채우는 건 **읽을 건 많은데 결론은 한 줄인 일** — 코드베이스 훑기, 긴 로그 판독입니다.

- 그런 일은 서브에이전트로 격리하고 **결론만** 받습니다. 파일 덤프가 아니라 "무엇이 어디에 있나".
...
- 검증은 **다른 문맥**에서 돕니다 — 왜 그렇게 만들었는지 모르는 채 결과만 봅니다.
- 검증자에게는 편집 도구를 주지 않습니다. 검증하다 슬쩍 고치는 걸 규율이 아니라 **구조로** 막습니다.

### 4. 테스트 초록불을 완료로 인정하지 않는다

시드나 픽스처가 실제 흐름을 우회하면, 통과해도 작동하지 않습니다.

- 완료 시점에 "실제로 확인했나"를 묻습니다. **막지는 않습니다** — 판단은 사람과 모델의 몫입니다.
...
- 훅은 두 종류뿐입니다. **되돌릴 수 없는 것만 차단**하고, 나머지는 한 줄 알립니다.
- 모델과 사고 깊이를 설정에 고정하지 않습니다 — 세션마다 사용자가 고릅니다. 고정하면 새 모델이 나와도 옛 세대에 묶입니다.
